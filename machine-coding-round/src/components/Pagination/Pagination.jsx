...
export default function Pagination() {
  ...
  return productsData?.products?.length ? (
    <div className="pagination--wrapper">
      <h2>
        PRODUCTS: <button onClick={handleAlert}>(Alert)</button>
      </h2>
      ...
      <div className="pagination--buttons" onClick={handlePageChange}>
        <button className={`pagination--btn`} disabled={page === 1} id="lt">
          &lt;
        </button>
        {productsData?.products?.length &&
          Array(Math.floor(productsData.total / productsData.limit))
            .fill(undefined)
            .map((_, idx) => (
              <button
        ...
        <button
          className="pagination--btn"
          disabled={page === productsData.total / productsData.limit}
          id="gt"
        >
          &gt;
        </button>
